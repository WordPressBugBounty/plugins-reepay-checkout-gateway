<?php
/**
 * Vipps MobilePay Recurring token
 *
 ...
 * @package Reepay\Checkout\Tokens
 */
class TokenReepayVR extends WC_Payment_Token {
	/**
	 * Token type
	 *
	 * @var string
	 */
	protected $type = 'Reepay_VR';
	...
	public static function wc_get_account_saved_payment_methods_list_item( array $item, WC_Payment_Token $payment_token ): array {
		if ( 'reepay_vipps_recurring' !== $payment_token->get_gateway_id() ) {
			return $item;
		}
	...
	public static function wc_account_payment_methods_column_method( array $method ) {
		if ( 'reepay_vipps_recurring' !== $method['method']['gateway'] ) {
			return;
		}
